...
function VerifyField(data: {
	...
	if (!verifyEmail(data.email, true)) throw new Error("Invalid email address");
	...
	return (
		<ScrollView>
			<View
				...
				<View>
					<Text  style={{color: theme.colors.onBackground}}  variant="headlineSmall">Welcome to Bibliotech</Text>
					<Text  style={{color: theme.colors.outline}}  variant="labelLarge">Sign up to continue</Text>
				</View>
				<View style={{ gap: 10 }}>
					<TextInput
					...
					<Button  style={{borderColor: theme.colors.primary}}  mode="outlined" onPress={() => router.replace("/auth/login")}>
						Log In
					</Button>
